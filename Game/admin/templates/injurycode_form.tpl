...
<ul class="snav">
    ...
    <li><a href="javascript:void(0);" class="sel"><img src="interface/icons/add.gif" alt="add" align="absmiddle" /> Update injury code </a></li>
</ul>
...
<fieldset>
      <table width="70%" cellpadding="3" cellspacing="0" border="0" class="std">
		...
		<tr>
			...
			{for $lop=1 to 125}
				<option value='{$lop}' {if ($t.code == $lop)} selected {/if}> Nature of Injury Code - {$lop} </option>
			{/for}
			</select>
			</td>
		</tr>
		<tr class='tble_header'>
			...
			<input type="checkbox" name="injurycode[guidelines][]" id="guidelines" value="{$item.id}" {if in_array($item.id,$guidelinecodes)} checked {/if}> {$item.title}
			</div>
			{/foreach}
