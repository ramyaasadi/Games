...
<h2>Injury code</h2>
<ul class="snav">
	<li><a href="?cat=injurycode" class="sel">Injury code List</a></li>
    <li><a href="?cat=injurycode&amp;action=add&amp;id=999999"><img src="interface/icons/add.gif" alt="add" align="absmiddle" /> Add injury code </a></li>
</ul>
<h3>Injury code<div style='float:right'> <span class='color_default'> Total injury codes </span>: {if $paging['records_all']} {$paging['records_all']}{else}{$list|@count} {/if}</div></h3>
<br />
{if $list}
...
<table cellpadding="3" cellspacing="0" class="std" border="0" width="100%" align="center" style='padding:10px 0; background: #f2f2f2'>
    <tr>
        <td style='text-align:right;'> Search by Injury title :  </td>
        <td><input type="text" id='tags' name="filter[string]" class="in" value="{$search}" /></td>
        <td><input type="submit" name="filter[apply]" value="Search" class="bt" /> {if $search != ""} <a href='?cat=injurycode'> Reset Search </a> {/if}</td>
    </tr>
	</table>
</form>
...
<tr class='tble_header'>
	...
	{if $order == 'id_desc'}
	<a href="?cat=injurycode&amp;action=list&amp;order=id_asc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay" style='padding-top:3px'>&caron;</a>
	{elseif $order == 'id_asc'}
	<a href="?cat=injurycode&amp;action=list&amp;order=id_desc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay" style='padding-top:3px'>&circ;</a>
	{else}
	<a href="?cat=injurycode&amp;action=list&amp;order=id_desc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay color_default">&circ;</a>
	<a href="?cat=injurycode&amp;action=list&amp;order=id_asc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay color_default">&caron;</a>
	{/if}
	</div>
	</td>
	<td>
	...
	<div style='float:left;'>
	{if $order == 'title_desc'}
	<a href="?cat=injurycode&amp;action=list&amp;order=title_asc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay" style='padding-top:3px'>&caron;</a>
	{elseif $order == 'title_asc'}
	<a href="?cat=injurycode&amp;action=list&amp;order=title_desc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay" style='padding-top:3px'>&circ;</a>
	{else}
	<a href="?cat=injurycode&amp;action=list&amp;order=title_desc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay color_default">&circ;</a>
	<a href="?cat=injurycode&amp;action=list&amp;order=title_asc{if $search != ""}&amp;searchterm={$search}{/if}" class="sorticondisplay color_default">&caron;</a>
	{/if}
	</div>
	</td>
	<td>Actions</td>
...
<tr{cycle values=', class="even"'}>
    ...
    <td><strong><a href="?cat=injurycode&amp;action=edit&amp;id={$item.id}">{$item.title} - [CODE : {$item.code}]</a></strong></td>
	...
	<div style="float:left; margin:2px;">{if $item.status == 'y'}<img src="interface/icons/i_star.png" title="Change Status" alt="Change Status" align="absmiddle" class="change_status" id="{$item.id}_n" /> {else} <img src="interface/icons/i_istar.png" title="Change Status" alt="Change Status" align="absmiddle" class="change_status" id="{$item.id}_y" />{/if}
        ...
        <a href="?cat=injurycode&amp;action=edit&amp;id={$item.id}"><img src="interface/icons/edit.gif" alt="edit" align="absmiddle" title="Edit Injury code" /></a>
		</div>
    </td>
</tr>
{/foreach}
...
$(document).on('click', '.change_status', function(){
	...
	{
		$.ajax({
					url: '?cat=injurycode&action=changestatus',
					type: 'POST',
...
$(function() {
    $( "#tags" ).autocomplete({
      source: "?cat=injurycode&action=showlist",
      minLength: 2,
